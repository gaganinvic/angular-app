...
export class UserPhotosService extends CrudService {
  ...
  public async getAllPhotos() {
    try {
      const userPhotos: IUserPhotos = await this.get<IUserPhotos>(`photo`);
      return userPhotos.photos;
    } catch (error) {
        console.error('Error during get User photos request', error);
        return Promise.reject(error);
    }
  }
